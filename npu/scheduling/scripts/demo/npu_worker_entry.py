...
def apply_acl_limits():
    ...
    try:
        ...
        if not ok:
            print(f"[npu_worker] init_device failed: {err}", file=sys.stderr)
            return
        ok, err = set_device_res_limit(device_id, ACL_RT_DEV_RES_CUBE_CORE, cube_limit)
        if not ok:
            print(f"[npu_worker] set_device_res_limit CUBE failed: {err}", file=sys.stderr)
        ok, err = set_device_res_limit(device_id, ACL_RT_DEV_RES_VECTOR_CORE, vector_limit)
        if not ok:
            print(f"[npu_worker] set_device_res_limit VECTOR failed: {err}", file=sys.stderr)
        ok, err = init_context(device_id)
        if not ok:
            print(f"[npu_worker] init_context failed: {err}", file=sys.stderr)
