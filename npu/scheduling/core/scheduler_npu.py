...
def select_optimal_NPU(candidate_npus, cube_req, cube_lim, vector_req, vector_lim, memory, omega, gamma, alpha, beta, delta):
...
def allocate_instance(data):
    ...
    with lock:
        if type_ == "inference":
            ...
            node_npus = {}
            for npu in active_npus:
                if all(npu.can_allocate(cube_req, cube_lim, vector_req, vector_lim, m, omega, gamma) for m in memory):
            ...
            for node_ip, npus_on_node in node_npus.items():
                if len(npus_on_node) >= n_npus_needed:
                    allocated = npus_on_node[:n_npus_needed]
                    break
            if not allocated and len(memory) >= n_npus_needed:
                for n in nodes_info:
                    new_on_node = [npu for npu in new_npus if npu.ip_address == n["ip"] and all(npu.can_allocate(cube_req, cube_lim, vector_req, vector_lim, m, omega, gamma) for m in memory)]
                    combined = node_npus.get(n["ip"], []) + new_on_node
                    if len(combined) >= n_npus_needed:
                        allocated = combined[:n_npus_needed]
