"""
INFless-R NPU Baseline Scheduler
==================================
Simulates INFless request-based allocation on Ascend NPU.
- Uses sm_requests (mapped to vector_req/cube_req) as primary resource signal.
- Best-fit scoring on single dimension (Vector req only, INFless-R style).
...
class NPU:
    ...
    def can_allocate(self, v_req, c_req, mem):
        # INFless-R: checks req for Vector/Cube; memory check
        return (
    ...
    def calculate_score(self, v_req, mem):
        # Single-dimension score: Vector req fragmentation (INFless-R style)
        v_usage = (self.current_vector_req + v_req) / self.total_vector
...
def select_optimal_npu(candidates, v_req, c_req, mem):
    best_score = float('inf')
    best = None
    for npu in candidates:
        if npu.can_allocate(v_req, c_req, mem):
            score = npu.calculate_score(v_req, mem)
            if score < best_score:
                best_score = score
                best = npu
    return best
...
def schedule_instance(data):
    ...
    if task_type in ('inference', 'llm-inference'):
        mem  = memory[0]
        # INFless-R: find co-located first, then active, then new
        best = select_optimal_npu(active_npus, v_req, c_req, mem)
        if best is None:
        ...
        for npu in active_npus:
            if all(npu.can_allocate(v_req, c_req, m) for m in memory):
...
if __name__ == '__main__':
    ...
    plt.plot(timestamps, npu_counts, '-.', color='blue', label='INFless-R-NPU')
    plt.xlabel('Time (minutes)')
    plt.ylabel('Active NPU cards')
    plt.title('INFless-R-NPU: Active NPU Count Over Time')
    plt.tight_layout()
    plt.savefig('logs/infless-r-npu-timeline.png', dpi=300)
