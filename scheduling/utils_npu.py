...
def start_instance(selected_npus, instance_id, image_name, service_name, args, allocated_port, ip_address):
    ...
    # 对于 LLM 模型，使用完整的 NPU 资源以避免计算精度问题
    if args.get("MODEL_PATH") and "qwen" in args.get("MODEL_PATH", "").lower():
        # Qwen 模型使用完整资源
        cube_lim = 20
        vector_lim = 40
        print(f"[NPU] LLM model detected, using full resources: cube={cube_lim}, vector={vector_lim}")
    else:
...
def stop_instance(service_name, instance_id, ip_address):
    ...
    try:
        ...
        result = subprocess.run(
            ["pgrep", "-f", f"{service_name}.*{instance_id}"],
        ...
        if result.returncode == 0:
            for p in result.stdout.strip().split('\n'):
                if p and p != str(pid):
                    try:
                        os.kill(int(p), 9)
                        print(f"[NPU] Killed additional process {p} for {instance_id}")
                    except:
                        pass
    except:
        pass
